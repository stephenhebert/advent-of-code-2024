...
// # 
// # issues:
// #  * i didn't understand the scoring system - I thought I needed 
// #    to find the number of possible paths, but I actually only 
// #    needed to find the number of endpoints that were reachable
// # 
// # 
// # reflections:
// # 
...
async function hikeTrail(position: Position, index: number) {
  ...
  return results.flat().reduce((acc, result) => {
    if (acc.some(({ x, y }) => x === result.x && y === result.y)) return acc
...
async function main() {
  ...
  const sum = results.map(result => result.length).reduce((acc, result) => acc + result, 0)
